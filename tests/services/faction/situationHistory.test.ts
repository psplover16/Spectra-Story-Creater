...
import { appendSituationHistory, readSituationHistory } from '@/services/faction/situationHistory'
...
const ROOT = path.join(tmpdir(), 'spectra-situation-history-tests')

describe('situationHistory', () => {
  let dir: string
  ...
  it('append 後可讀回，且 UTF-8 無 BOM、含時戳', async () => {
    ...
    const list = await readSituationHistory(dir)
    expect(list).toHaveLength(1)
    expect(list[0]?.at).toBe('2026-05-13T00:00:00.000Z')
